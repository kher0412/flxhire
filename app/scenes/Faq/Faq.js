...
import { PageWrapper, PageContainer, PageHeader, Accordion, AnimBox } from 'components'
import { findIndex, matches } from 'lodash'
...
class Faq extends React.Component {
  ...
  handleClick = (category, questionId) => {
    const faq = [...this.state.faq]
    const categoryCopy = { ...category }
    const questionIndex = findIndex(categoryCopy.questions, matches({ id: questionId }))
    const categoryIndex = findIndex(faq, matches({ id: categoryCopy.id }))
    categoryCopy.questions[questionIndex].open = !categoryCopy.questions[questionIndex].open
    faq[categoryIndex] = categoryCopy
    this.setState({ faq: faq })
  }

  content = () => {
    return this.state.faq.map((category, categoryIndex) => (
      <div key={category.id}>
        {categoryIndex > 0 && (
        ...
        <AnimBox offset={-240}>
          <AnimBox grow delay={200} offset={-10000}>
          ...
          {category.questions.map((question, questionIndex) => (
            <AnimBox heavySlide delay={600 + questionIndex * 70} offset={-10000}>
              <Accordion key={question.id} title={question.question} defaultOpen={question.defaultOpen}>
                {question.answer}
              </Accordion>
            </AnimBox>
          ))}
    ...
    return (
      <div>
        ...
        <PageContainer>
          <Helmet>
            <title>FAQ</title>
            <meta name="description" content="Answering questions like why You should use Flexhire, how much does it cost and how do you get paid." />
            <meta property="og:title" content="FAQ - Flexhire" />
            <meta property="og:description" content="Answers to common questions such as why you should use Flexhire, how much does it cost and how we handle payments" />
          </Helmet>
