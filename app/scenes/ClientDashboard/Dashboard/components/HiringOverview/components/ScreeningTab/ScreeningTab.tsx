  ...
  render() {
    ...
    return (
      <Grid container spacing={2} className={styles.list}>
        ...
        {!error && !loading && applications.slice(0, 3).map(application => (
          <Grid key={application.id} item xs={12} md={3}>
            <DataCard
              ...
              href={`/client/hire?${buildQueryParams({ focus: application.freelancer.profile.slug, tab: 'screening', job: application.job_slug })}`}
            />
          </Grid>
        ))}
        ...
        <Grid item xs={12} md={3}>
          <ViewAllButton href="/client/hire?tab=screening" />
        </Grid>
      </Grid>
    )
  }
