...
export default function Question(props) {
  const { question, user, handleUse, onVideoChanged } = props
      ...
      {isLargeScreen => (
        <Card raised>
          <CardHeader
            ...
            style={isLargeScreen ? { padding: 48 } : undefined}
            title={(
          ...
          {question.description && (
            <CardContent
              style={{
                padding: isLargeScreen ? '0 48px 19px 48px' : undefined,
                fontSize: 16,
                color: '#8E97A3',
                lineHeight: '31px',
              }}
            >
              Question goal: {question.description}
            </CardContent>
          )}

          <div style={isLargeScreen ? { padding: '0 48px' } : { padding: '0 12px' }}>
            <QuestionSubheader
              freelancer_types={question.freelancer_types}
              freelancer_subtypes={question.freelancer_subtypes}
